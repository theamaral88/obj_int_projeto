...
void reconnect(){
  ...
  while (!client.connected()){
    ...
    if (client.connect(clientId.c_str(),"72aa269b@228","dc3a7b397429", willTopic, willQoS, willRetain, willMessage, true)){
      ...
      const char* welcomeMsg = "ESP32 CAM Conectado!";
      client.publish(willTopic, welcomeMsg);
    } else {
      Serial.print("Falha na conexão, tentando novamente em 5s...");
      delay(5000);
    }
  }
}
...
void setup() {
  ...
  if (!espClient.connect(mqtt_server, 1883)){
...
void loop() {
  ...
  // Publica mensagem a cada 30s
  if (millis() - t > 30000){
    t = millis();
    msg = "Tempo ativo " + String(t / 1000) + "s";
    Serial.println("Publicado: " + msg);
    client.publish("MackMQTT-Emerson", msg.c_str());
  }
